...
require_relative 'Conecto_types'
...
class ConectoSDK
  ...
  def initialize(options = {})
    ...
    utility = ConectoUtility.new
    @_utility = utility

    config = ConectoConfig.shared_config
    ...
    feature_opts = ConectoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ConectoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ConectoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ConectoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ConectoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ConectoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ConectoError.new(
        "#{op}_allow",
        "ConectoSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ConectoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ConectoError.new(
        "graphql_error", "ConectoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Conversation.list / client.Conversation.load({ "id" => ... })
  def Conversation(data = nil)
    require_relative 'entity/conversation_entity'
    ConversationEntity.new(self, data)
  end
  ...
  # Canonical facade: client.Visitor.list / client.Visitor.load({ "id" => ... })
  def Visitor(data = nil)
    require_relative 'entity/visitor_entity'
    VisitorEntity.new(self, data)
  end
