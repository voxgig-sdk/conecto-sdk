...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ConectoOmni.make_runner(TEST_JSON_FILE, ConectoSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ConectoOmni.errify(err)
    end
