...
class ActionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = action_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "action." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CONECTO_TEST_ACTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    action_ref01_ent = client.Action(nil)
...
def action_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "action", "ActionTestData.json")
  ...
  entid_env_raw = ENV["CONECTO_TEST_ACTION_ENTID"]
