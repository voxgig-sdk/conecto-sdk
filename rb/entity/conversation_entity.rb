...
class ConversationEntity
  ...
  def make
    opts = @_entopts.dup
    ConversationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ConectoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Conversation, Hash] the current Conversation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ConectoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Conversation fields)
  def match_get
  ...
  #
  # @param reqmatch [ConversationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Conversation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Conversation, Hash] the loaded Conversation; raises ConectoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ConectoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ConversationListMatch, Hash, nil] match filter (any subset of
  #   Conversation fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Conversation>, Array] the matching Conversation items; raises ConectoError on failure
  ...
  #
  # @param reqdata [ConversationCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Conversation, Hash] the created Conversation; raises ConectoError on failure
  ...
  #
  # @param reqdata [ConversationUpdateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Conversation, Hash] the updated Conversation; raises ConectoError on failure
