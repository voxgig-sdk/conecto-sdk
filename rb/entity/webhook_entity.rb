...
class WebhookEntity
  ...
  def make
    opts = @_entopts.dup
    WebhookEntity.new(@_client, opts)
  end
  ...
  #
  # @param reqmatch [WebhookLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Webhook.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Webhook, Hash] the loaded Webhook; raises ConectoError on failure
  ...
  #
  # @param reqmatch [WebhookListMatch, Hash, nil] match filter (any subset of
  #   Webhook fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Webhook>, Array] the matching Webhook items; raises ConectoError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Webhook, Hash] the created Webhook; raises ConectoError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Webhook, Hash] the removed Webhook; raises ConectoError on failure
