...
module ConectoUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ConectoContext.new({}, nil)
    end
    op = ctx.op || ConectoOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ConectoResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ConectoError) ? err.msg : err.to_s
    ...
    sdk_err = ConectoError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ConectoError)
