...
function isRunnable(code) {
    return (/new\s+ConectoSDK\b/.test(code) ||
        /\bConectoSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ConectoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ConectoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
