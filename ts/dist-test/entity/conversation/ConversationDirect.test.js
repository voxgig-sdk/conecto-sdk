...
(0, node_test_1.describe)('ConversationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-conversation', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-conversation', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-conversation', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-conversation', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CONECTO_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ConectoSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.CONECTO_APIKEY,
        }));
        let idmap = env['CONECTO_TEST_CONVERSATION_ENTID'];
    ...
    const client = new __1.ConectoSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ConversationDirect.test.js.map