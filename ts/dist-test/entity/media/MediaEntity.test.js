...
(0, node_test_1.describe)('MediaEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'media.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['media01', 'media02', 'media03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CONECTO_TEST_MEDIA_ENTID'];
