...
(0, node_test_1.describe)('CredentialEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'credential.' + op, live))
                return;
        }
        ...
        (0, node_assert_1.default)(null != credential_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    let idmap = transform(['credential01', 'credential02', 'credential03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CONECTO_TEST_CREDENTIAL_ENTID'];
