...
boru-driven command-line client **and** interactive REPL for the Conecto
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/conecto-cli)
make build
...
CONECTO_BASE=https://api.example.com ./conecto-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/conecto-cli
   ```
...
Run with no arguments to open a REPL (prompt `conecto>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/conecto-cli`. |
...
(`./conecto-cli <expr>`) and interactively (the REPL), and expressions compose the
