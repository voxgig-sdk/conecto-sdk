...
class ConectoMakeError
{
    public static function call(?ConectoContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ConectoContext([], null);
        }
        $op = $ctx->op ?? new ConectoOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ConectoResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ConectoError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ConectoError('', $msg, $ctx);
        ...
        if ($err instanceof ConectoError) {
