...
{
    private static function utility(): ConectoUtility
    {
        return new ConectoUtility();
    }
    ...
    private static function ctx(array $over = []): ConectoContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ConectoContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ConectoOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ConectoError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ConectoOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ConectoMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ConectoOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ConectoMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ConectoOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ConectoMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ConectoSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ConectoMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ConectoResponse([]),
            'result' => new ConectoResult([]),
        ]);
        [, $err] = ConectoMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ConectoSpec([]),
            'response' => null,
            'result' => new ConectoResult([]),
        ]);
        [, $err] = ConectoMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ConectoSpec(['step' => 's']),
            'response' => new ConectoResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ConectoResult([]),
        ]);
        [, $err] = ConectoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ConectoSpec(['step' => 's']),
            'response' => new ConectoResponse(self::resp(200, ['v' => 1])),
            'result' => new ConectoResult([]),
        ]);
        [, $err] = ConectoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ConectoSpec(['step' => 's']),
            'response' => new ConectoResponse(self::resp(200, ['v' => 2])),
            'result' => new ConectoResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ConectoMakeResponse::call($ctx);
    ...
    {
        $preset = new ConectoResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ConectoSpec([]),
            'response' => new ConectoResponse([]),
            'result' => new ConectoResult([]),
        ...
        [$got, $err] = ConectoMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ConectoResult([])]);
        [, $err] = ConectoMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ConectoSpec([]), 'result' => null]);
        [, $err] = ConectoMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ConectoSpec(['step' => 's']),
            'result' => new ConectoResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ConectoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ConectoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ConectoSpec(['step' => 's']),
            'result' => new ConectoResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ConectoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ConectoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ConectoResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ConectoSpec([]),
            'result' => new ConectoResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ConectoMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ConectoMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ConectoError('boom', 'boom');
        $utility->fetcher = function (ConectoContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ConectoSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ConectoMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ConectoContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ConectoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ConectoResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ConectoContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ConectoSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ConectoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ConectoResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ConectoSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ConectoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ConectoMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ConectoMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ConectoResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ConectoDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ConectoResult(['ok' => false])]);
        $this->expectException(ConectoError::class);
        ConectoDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ConectoResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ConectoMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ConectoMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ConectoBaseFeature();
        $b = new ConectoBaseFeature();
        ConectoFeatureAdd::call($ctx, $a);
        ConectoFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ConectoBaseFeature
    {
        $f = new ConectoBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ConectoFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ConectoContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ConectoSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ConectoPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ConectoResponse(['headers' => null]),
            'result' => new ConectoResult([]),
        ]);
        ConectoResultHeaders::call($ctx);
