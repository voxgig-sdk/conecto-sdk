...
// Conecto SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ConectoOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ConectoOmni::structfix($now, Struct::undef()))
                        !== json_encode(ConectoOmni::structfix($val, Struct::undef()))) {
