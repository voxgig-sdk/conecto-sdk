...
// Conecto SDK Conversation entity
...
class ConversationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ConversationEntity($this->_client, $opts);
    }

    /**
     * @param Conversation|array $args Conversation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ConectoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Conversation|array The current Conversation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Conversation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ConectoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Conversation fields).
     */
     ...
     *
     * @param ConversationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ConversationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Conversation|array The loaded Conversation as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ConectoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Conversation items matching the given filter.
     *
     * @param ConversationListMatch|array|null $reqmatch Match filter (any subset
     *   of Conversation fields) as an assoc-array; ConversationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Conversation[]|array A list of Conversation items as assoc-arrays at
     *   the SDK boundary; throws ConectoError on failure (item-5 convention).
     */
     ...
     *
     * @param ConversationCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ConversationCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Conversation|array The created Conversation as an assoc-array at the
     *   SDK boundary; throws ConectoError on failure (item-5 convention).
     */
     ...
     *
     * @param ConversationUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ConversationUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Conversation|array The updated Conversation as an assoc-array at the
     *   SDK boundary; throws ConectoError on failure (item-5 convention).
     */
