...
class MediaEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CONECTO_TEST_MEDIA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $media_ref01_data_result = $media_ref01_ent->create($media_ref01_data, null);
        $media_ref01_data = Helpers::to_map(is_object($media_ref01_data_result) && method_exists($media_ref01_data_result, 'data_get') ? $media_ref01_data_result->data_get() : $media_ref01_data_result);
        $this->assertNotNull($media_ref01_data);
...
function media_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CONECTO_TEST_MEDIA_ENTID");
