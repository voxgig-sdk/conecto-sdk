...
class CredentialEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CONECTO_TEST_CREDENTIAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $credential_ref01_data = null;
        if (count($credential_ref01_data_raw) > 0) {
        ...
        $credential_ref01_data_dt0_loaded = $credential_ref01_ent->load($credential_ref01_match_dt0, null);
        $this->assertNotNull($credential_ref01_data_dt0_loaded);
...
function credential_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CONECTO_TEST_CREDENTIAL_ENTID");
