...
class FtCtrl extends ConectoControl
{
...
class FtClient
{
    ...
    private ConectoUtility $utility;

    public function __construct(ConectoUtility $utility, array $options)
    {
    ...
    public function get_utility(): ConectoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ConectoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new ConectoUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (ConectoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ConectoConfig::make_feature($name);
    ...
    private static function buildUrl(ConectoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ConectoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ConectoResponse($fetched);
        }
    ...
    private function fail(ConectoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ConectoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
