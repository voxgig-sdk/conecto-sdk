...
class ActionEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Action(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = action_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "action." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CONECTO_TEST_ACTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $action_ref01_ent = $client->Action(null);
...
function action_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/action/ActionTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["action01", "action02", "action03", "integration01", "integration02", "integration03", "slug01"] as $k) {
    ...
    $entid_env_raw = getenv("CONECTO_TEST_ACTION_ENTID");
