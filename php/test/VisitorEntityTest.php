...
class VisitorEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CONECTO_TEST_VISITOR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $visitor_ref01_data_result = $visitor_ref01_ent->create($visitor_ref01_data, null);
        $visitor_ref01_data = Helpers::to_map(is_object($visitor_ref01_data_result) && method_exists($visitor_ref01_data_result, 'data_get') ? $visitor_ref01_data_result->data_get() : $visitor_ref01_data_result);
        $this->assertNotNull($visitor_ref01_data);
...
function visitor_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CONECTO_TEST_VISITOR_ENTID");
