// Command Conecto-mcp is an MCP server exposing the Conecto
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Conecto_list  — list records of an entity
//	Conecto_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: CONECTO_APIKEY carries the API key and
	// CONECTO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CONECTO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CONECTO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewConectoSDK(opts)
