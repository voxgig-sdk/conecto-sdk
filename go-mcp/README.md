...
[MCP](https://modelcontextprotocol.io) server exposing the Conecto SDK as
two agent tools — `conecto_list` and `conecto_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/conecto-mcp)
make build

# 2. Provide credentials via the environment
export CONECTO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user conecto \
  -- /absolute/path/to/conecto-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./conecto-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// conecto_list: first page of records
{ "entity": "contact" }
{ "entity": "contact", "query": { } }

// conecto_load: one record by id
{ "entity": "conversation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/conecto-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user conecto \
     -- "$PWD"/dist/*/conecto-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `conecto_list` and `conecto_load` tools now appear
   in new sessions. Ask the agent to *"list contact using conecto"*
   and it calls `conecto_list` with `{"entity":"contact"}`.
...
```sh
export CONECTO_APIKEY=sk_live_xxx            # API key
export CONECTO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `conecto_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `conecto_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CONECTO_APIKEY` | API key sent with every request. |
| `CONECTO_BASE` | Optional override of the API base URL. |
...
```sh
./conecto-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"conecto_load","arguments":{"entity":"conversation","query":{"id":1}}}}'
```
