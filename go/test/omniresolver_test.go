...
import (
	...
	omni "github.com/voxgig-sdk/conecto-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ConectoError); is {
				out := map[string]any{
					"name":    "ConectoError",
...
func omniCtx(arg any, client *sdk.ConectoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ConectoSDK); is {
			client = live
