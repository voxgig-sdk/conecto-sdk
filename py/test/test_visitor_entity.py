    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CONECTO_TEST_VISITOR_ENTID JSON to run live")
        client = setup["client"]
        ...
        visitor_ref01_data = helpers.to_map(runner.entity_data(visitor_ref01_ent.create(visitor_ref01_data, None)))
        assert visitor_ref01_data is not None
...
def _visitor_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CONECTO_TEST_VISITOR_ENTID"))
