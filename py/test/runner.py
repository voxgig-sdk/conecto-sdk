...
class ConectoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ConectoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ConectoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ConectoTestRunner.getenv("CONECTO_TEST_LIVE")
        override = ConectoTestRunner.getenv("CONECTO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ConectoTestRunner.getenv(key)
        ...
        explain = ConectoTestRunner.getenv("CONECTO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CONECTO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ConectoTestRunner._test_control is not None:
            return ConectoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ConectoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ConectoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ConectoTestRunner.load_test_control()
...
def load_env_local():
    ConectoTestRunner.load_env_local()


def env_override(m):
    return ConectoTestRunner.env_override(m)


def entity_data(v):
    return ConectoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ConectoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ConectoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ConectoTestRunner.load_test_control()


def live_client_options():
    return ConectoTestRunner.live_client_options()


def live_delay_ms():
    return ConectoTestRunner.live_delay_ms()
