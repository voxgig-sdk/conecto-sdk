...
describe("ConversationDirect", function()
  it("should direct-list-conversation", function()
    local setup = conversation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-conversation", setup.live and "live" or "unit")
  ...
  it("should direct-load-conversation", function()
    local setup = conversation_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-conversation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function conversation_direct_setup(mockres)
