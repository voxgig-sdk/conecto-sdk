...
describe("WebhookDirect", function()
  it("should direct-list-webhook", function()
    local setup = webhook_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-webhook", setup.live and "live" or "unit")
  ...
  it("should direct-load-webhook", function()
    local setup = webhook_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-webhook", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function webhook_direct_setup(mockres)
