...
local IntegrationEntity = {}
IntegrationEntity.__index = IntegrationEntity


function IntegrationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IntegrationEntity)
  self._name = "integration"
  self._client = client
...
function IntegrationEntity:get_name()
  return self._name
end


function IntegrationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IntegrationEntity.new(self._client, opts)
end
...
function IntegrationEntity:mark_deleted()
  self._deleted = true
end


function IntegrationEntity:deleted()
  return true == self._deleted
end


function IntegrationEntity:data_set(args)
  if args ~= nil then
...
function IntegrationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IntegrationEntity:match_set(args)
  if args ~= nil then
...
function IntegrationEntity:match_get()
...
function IntegrationEntity:stream(action, args, callopts)
...
---@param reqmatch IntegrationLoadMatch
---@param ctrl? table
---@return Integration
---@return string? err
function IntegrationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch IntegrationListMatch
---@param ctrl? table
---@return Integration[]
---@return string? err
function IntegrationEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata IntegrationCreateData
---@param ctrl? table
---@return Integration
---@return string? err
function IntegrationEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function IntegrationEntity:_run_op(ctx, post_done)
  local utility = self._utility
