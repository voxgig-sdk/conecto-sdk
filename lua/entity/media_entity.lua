...
local MediaEntity = {}
MediaEntity.__index = MediaEntity


function MediaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MediaEntity)
  self._name = "media"
  self._client = client
...
function MediaEntity:get_name()
  return self._name
end


function MediaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MediaEntity.new(self._client, opts)
end
...
function MediaEntity:mark_deleted()
  self._deleted = true
end


function MediaEntity:deleted()
  return true == self._deleted
end


function MediaEntity:data_set(args)
  if args ~= nil then
...
function MediaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MediaEntity:match_set(args)
  if args ~= nil then
...
function MediaEntity:match_get()
...
function MediaEntity:stream(action, args, callopts)
...
---@param reqdata MediaCreateData
---@param ctrl? table
---@return Media
---@return string? err
function MediaEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function MediaEntity:_run_op(ctx, post_done)
  local utility = self._utility
