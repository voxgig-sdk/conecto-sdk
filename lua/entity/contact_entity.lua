...
local ContactEntity = {}
ContactEntity.__index = ContactEntity


function ContactEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContactEntity)
  self._name = "contact"
  self._client = client
...
function ContactEntity:get_name()
  return self._name
end


function ContactEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContactEntity.new(self._client, opts)
end
...
function ContactEntity:mark_deleted()
  self._deleted = true
end


function ContactEntity:deleted()
  return true == self._deleted
end


function ContactEntity:data_set(args)
  if args ~= nil then
...
function ContactEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContactEntity:match_set(args)
  if args ~= nil then
...
function ContactEntity:match_get()
...
function ContactEntity:stream(action, args, callopts)
...
---@param reqmatch ContactListMatch
---@param ctrl? table
---@return Contact[]
---@return string? err
function ContactEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ContactCreateData
---@param ctrl? table
---@return Contact
---@return string? err
function ContactEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ContactEntity:_run_op(ctx, post_done)
  local utility = self._utility
