...
function MessageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MessageEntity)
  self._name = "message"
  self._client = client
...
function MessageEntity:get_name()
  return self._name
end


function MessageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MessageEntity.new(self._client, opts)
end
...
function MessageEntity:mark_deleted()
  self._deleted = true
end


function MessageEntity:deleted()
  return true == self._deleted
end


function MessageEntity:data_set(args)
  if args ~= nil then
...
function MessageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MessageEntity:match_set(args)
  if args ~= nil then
...
function MessageEntity:match_get()
...
function MessageEntity:stream(action, args, callopts)
...
function MessageEntity:_run_op(ctx, post_done)
  local utility = self._utility
