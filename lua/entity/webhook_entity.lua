...
local WebhookEntity = {}
WebhookEntity.__index = WebhookEntity


function WebhookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhookEntity)
  self._name = "webhook"
  self._client = client
...
function WebhookEntity:get_name()
  return self._name
end


function WebhookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhookEntity.new(self._client, opts)
end
...
function WebhookEntity:mark_deleted()
  self._deleted = true
end


function WebhookEntity:deleted()
  return true == self._deleted
end


function WebhookEntity:data_set(args)
  if args ~= nil then
...
function WebhookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhookEntity:match_set(args)
  if args ~= nil then
...
function WebhookEntity:match_get()
...
function WebhookEntity:stream(action, args, callopts)
...
function WebhookEntity:_run_op(ctx, post_done)
  local utility = self._utility
