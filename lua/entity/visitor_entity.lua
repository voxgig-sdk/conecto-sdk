...
local VisitorEntity = {}
VisitorEntity.__index = VisitorEntity


function VisitorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VisitorEntity)
  self._name = "visitor"
  self._client = client
...
function VisitorEntity:get_name()
  return self._name
end


function VisitorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VisitorEntity.new(self._client, opts)
end
...
function VisitorEntity:mark_deleted()
  self._deleted = true
end


function VisitorEntity:deleted()
  return true == self._deleted
end


function VisitorEntity:data_set(args)
  if args ~= nil then
...
function VisitorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VisitorEntity:match_set(args)
  if args ~= nil then
...
function VisitorEntity:match_get()
...
function VisitorEntity:stream(action, args, callopts)
...
---@param reqdata VisitorCreateData
---@param ctrl? table
---@return Visitor
---@return string? err
function VisitorEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function VisitorEntity:_run_op(ctx, post_done)
  local utility = self._utility
