...
local SchemaEntity = {}
SchemaEntity.__index = SchemaEntity


function SchemaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SchemaEntity)
  self._name = "schema"
  self._client = client
...
function SchemaEntity:get_name()
  return self._name
end


function SchemaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SchemaEntity.new(self._client, opts)
end
...
function SchemaEntity:mark_deleted()
  self._deleted = true
end


function SchemaEntity:deleted()
  return true == self._deleted
end


function SchemaEntity:data_set(args)
  if args ~= nil then
...
function SchemaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SchemaEntity:match_set(args)
  if args ~= nil then
...
function SchemaEntity:match_get()
...
function SchemaEntity:stream(action, args, callopts)
...
---@param reqmatch SchemaLoadMatch
---@param ctrl? table
---@return Schema
---@return string? err
function SchemaEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SchemaEntity:_run_op(ctx, post_done)
  local utility = self._utility
