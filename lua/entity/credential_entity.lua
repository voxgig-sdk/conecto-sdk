...
local CredentialEntity = {}
CredentialEntity.__index = CredentialEntity


function CredentialEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CredentialEntity)
  self._name = "credential"
  self._client = client
...
function CredentialEntity:get_name()
  return self._name
end


function CredentialEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CredentialEntity.new(self._client, opts)
end
...
function CredentialEntity:mark_deleted()
  self._deleted = true
end


function CredentialEntity:deleted()
  return true == self._deleted
end


function CredentialEntity:data_set(args)
  if args ~= nil then
...
function CredentialEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CredentialEntity:match_set(args)
  if args ~= nil then
...
function CredentialEntity:match_get()
...
function CredentialEntity:stream(action, args, callopts)
...
---@param reqmatch CredentialLoadMatch
---@param ctrl? table
---@return Credential
---@return string? err
function CredentialEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CredentialEntity:_run_op(ctx, post_done)
  local utility = self._utility
